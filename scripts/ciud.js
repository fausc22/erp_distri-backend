  ...
  async procesarLoteClientes(clientes) {
    for (const cliente of clientes) {
      try {
        ...
        let ciudadNombre = 'GENERAL PICO'; // Nombre por defecto
        let fuente = 'default';

        if (resultado) {
          ciudadId = resultado.ciudad.id;
          ciudadNombre = resultado.ciudad.nombreOriginal;
        ...
        // Actualizar tanto ciudad_id como ciudad en la base de datos
        await this.connection.execute(
          'UPDATE clientes SET ciudad_id = ?, ciudad = ? WHERE id = ?',
          [ciudadId, ciudadNombre, cliente.id]
        );
