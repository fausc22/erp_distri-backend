...
const SQL_LOCK_ERRORS = new Set(['ER_LOCK_DEADLOCK', 'ER_LOCK_WAIT_TIMEOUT']);
const SQL_DUPLICATE_ERRORS = new Set(['ER_DUP_ENTRY']);

const mapearErrorCritico = (error, fallbackMessage) => {
  if (error instanceof AppError) {
    return error;
  }
  if (SQL_LOCK_ERRORS.has(error?.code)) {
    return new AppError(
      'No se pudo completar la operación por bloqueo concurrente. Reintente.',
      'LOCK_CONFLICT',
      409,
      { sqlCode: error.code }
    );
  }
  if (SQL_DUPLICATE_ERRORS.has(error?.code)) {
    return new AppError(
      'No se pudo completar la operación por datos duplicados.',
      'DUPLICATE_ENTRY',
      409,
      { sqlCode: error.code }
    );
  }
  return new AppError(fallbackMessage, 'INTERNAL_ERROR', 500, { sqlCode: error?.code || null });
};

const responderErrorCritico = (res, error, fallbackMessage, logMessage) => {
  const appError = mapearErrorCritico(error, fallbackMessage);
...
const registrarMovimiento = async (req, res) => {
  ...
  try {
    const resultado = await withTransaction(async (conn) => {
      const cuenta = await fondosRepository.obtenerCuentaPorIdForUpdate(conn, cuenta_id);
      if (!cuenta) {
        throw new AppError('Cuenta no encontrada', 'NOT_FOUND', 404);
      }

      const mov = await fondosRepository.registrarMovimiento(conn, {
        cuenta_id,
        tipo,
        origen,
        monto,
      ...
      const delta = tipo === 'INGRESO' ? Math.abs(parseFloat(monto)) : -Math.abs(parseFloat(monto));
      await fondosRepository.actualizarSaldo(conn, cuenta_id, delta);
    ...
    return responderErrorCritico(
      res,
      error,
      'Error al registrar el movimiento',
      'Error al registrar movimiento (transaccion revertida):'
    );
  }
};
...
const realizarTransferencia = async (req, res) => {
  ...
  try {
    await withTransaction(async (conn) => {
      const [idA, idB] = [Number(cuenta_origen), Number(cuenta_destino)].sort((a, b) => a - b);
      const primeraCuenta = await fondosRepository.obtenerCuentaPorIdForUpdate(conn, idA);
      const segundaCuenta = await fondosRepository.obtenerCuentaPorIdForUpdate(conn, idB);

      if (!primeraCuenta || !segundaCuenta) {
        throw new AppError('Cuenta de origen o destino no encontrada', 'NOT_FOUND', 404);
      }

      const cuentaOrigen = Number(cuenta_origen) === idA ? primeraCuenta : segundaCuenta;
      if (parseFloat(cuentaOrigen.saldo) < parseFloat(monto)) {
        throw new AppError('Saldo insuficiente en la cuenta de origen', 'INSUFFICIENT_FUNDS', 400);
      }

      await fondosRepository.transferir(conn, cuenta_origen, cuenta_destino, monto, {
    ...
    return responderErrorCritico(
      ...
      'Error en transferencia (transaccion revertida):'
    );
  }
};
...
const obtenerBalanceGeneral = async (req, res) => {
  ...
  try {
    const results = await reportesRepository.obtenerBalanceGeneral({ anio, desde, hasta });
...
const obtenerBalancePorCuenta = (req, res) => {
  ...
  if (desde && hasta) {
    filtroFecha = 'WHERE DATE(mf.fecha) BETWEEN ? AND ?';
    params.push(desde, hasta);
  } else if (desde) {
    filtroFecha = 'WHERE DATE(mf.fecha) >= ?';
    params.push(desde);
  } else if (hasta) {
    filtroFecha = 'WHERE DATE(mf.fecha) <= ?';
    params.push(hasta);
...
const obtenerFlujoDeFondos = async (req, res) => {
  const { desde, hasta, cuenta_id } = req.query;
  try {
    const results = await reportesRepository.obtenerFlujoDeFondos({ desde, hasta, cuenta_id });
  ...
  try {
    ...
    const query = `
      ...
      SELECT
        ...
        ROUND(SUM(COALESCE(cv.ganancia_neta, 0)), 2) AS ganancia_generada
  ...
  try {
    ...
    const query = `
      SELECT
        vc.producto_id,
        vc.producto_nombre,
        ROUND(SUM(${CANTIDAD_LINEA('vc', 'v')}), 2) AS total_vendida,
        ROUND(SUM(${INGRESO_LINEA('vc', 'v')}), 2) AS ingresos_netos,
  ...
  try {
    ...
    const query = `
      ...
      costo_por_venta AS (
        SELECT
          v.id AS venta_id,
          ROUND(SUM(${COSTO_LINEA('vc', 'p', 'v')}), 2) AS costo_neto,
          ROUND(SUM(${GANANCIA_LINEA('vc', 'p', 'v')}), 2) AS ganancia_neta
        FROM ventas v
        JOIN ventas_cont vc ON vc.venta_id = v.id
      ...
      SELECT
        ...
        ROUND(SUM(COALESCE(cv.ganancia_neta, 0)), 2) AS ganancia_estimada,
  ...
  try {
    ...
    const query = `
      SELECT
        ...
        ROUND(SUM(${INGRESO_LINEA('vc', 'v')}), 2) AS ingresos_producto,
        ROUND(SUM(${GANANCIA_LINEA('vc', 'p', 'v')}), 2) AS ganancia_total,
        (
          CASE
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        vc.producto_nombre,
        ROUND(AVG(vc.precio), 2) AS precio_promedio,
        ROUND(SUM(${CANTIDAD_LINEA('vc', 'v')}), 2) AS cantidad_total_vendida,
        ROUND(SUM(${INGRESO_LINEA('vc', 'v')}), 2) AS ingresos_producto,
        ROUND(SUM(${COSTO_LINEA('vc', 'p', 'v')}), 2) AS costo_producto,
        ROUND(SUM(${GANANCIA_LINEA('vc', 'p', 'v')}), 2) AS ganancia_estimada,
        ROUND(
          CASE WHEN SUM(${INGRESO_LINEA('vc', 'v')}) = 0 THEN 0
               ELSE (SUM(${GANANCIA_LINEA('vc', 'p', 'v')}) / SUM(${INGRESO_LINEA('vc', 'v')})) * 100
          END, 2
      ...
      SELECT
        ...
        ROUND(
          CASE WHEN SUM(vf.ingreso_neto) = 0 THEN 0
               ELSE (SUM(COALESCE(cv.ganancia_neta, 0)) / SUM(vf.ingreso_neto)) * 100
          END, 2
        ) AS margen_promedio
      FROM ventas_filtradas vf
      LEFT JOIN costo_por_venta cv ON cv.venta_id = vf.id
...
const obtenerResumenFinanciero = async (req, res) => {
  try {
    ...
    const { ventasRow, costosRow, comprasRow, gastosRow } =
      await reportesRepository.obtenerResumenFinanciero(filtros);
    ...
    const gananciaBruta = Number(costosRow.ganancia_estimada || ingresosTotales - costoVentas);
...
const obtenerDashboardSimplificado = async (req, res) => {
  try {
    ...
    const {
      ...
      ciudades,
      cuentas
    } = await reportesRepository.obtenerDashboardSimplificado(filtros);
    ...
    const margenBruto = Number(costosRow.ganancia_estimada || ventasMonto - costoVentas);
...
const obtenerDatosReporteGerencial = async (desde, hasta) => {
  ...
  // 3) Ciudades ordenadas por cantidad de ventas
